...
class UNet(nn.Module):
    def __init__(self, ngf=32, input_channel=3, output_channel=3):  # Increased ngf for greater capacity
    ...
    def forward(self, x, use_sigmoid=False):  # Removed sigmoid activation
        x = self.conv_init(x)
        x = self.init(x)
